...
> = {
  ...
  inputSchema: {
    type: "object",
    required: ["clientName", "offerSummary"],
    properties: {
  ...
  handler: (input, context) => {
    const concessions = Array.isArray(input.concessions) ? input.concessions : [];
    const nameRedaction = redactSensitiveText(input.clientName, {
      allowPii: false,
      redactKeys: context.config.audit.redactKeys
    });
    const offerRedaction = redactSensitiveText(input.offerSummary, {
      allowPii: false,
      redactKeys: context.config.audit.redactKeys
    });
    const plan = {
